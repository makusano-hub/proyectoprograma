...
void dibuMenu(HUD *hud, Jugador *j){
    ...
    al_draw_scaled_bitmap(hud->iconoVida,0,0,al_get_bitmap_width(hud->iconoVida),al_get_bitmap_height(hud->iconoVida),xMenu+20,55,32,32,0);

    al_draw_textf(hud->fuente,al_map_rgb(255,80,80), xMenu+20,90,0,"Oro: %d",j->oro);
    al_draw_scaled_bitmap(hud->iconoOro,0,0,al_get_bitmap_width(hud->iconoOro),al_get_bitmap_height(hud->iconoOro),xMenu+20,95,32,32,0);
       

    al_draw_textf(hud->fuente,al_map_rgb(255,80,80), xMenu+20,120,0,"Madera: %d",j->madera);
    al_draw_scaled_bitmap(hud->iconoMadera,0,0,al_get_bitmap_width(hud->iconoMadera),al_get_bitmap_height(hud->iconoMadera),xMenu+20,135,32,32,0);
